...
✅ **Resuelto (10 septiembre 2026)**: la excepción de arriba en realidad no era solo de `auto/coupe/frente/` — el template estándar de Frente (las "9 zonas estándar" reutilizadas en toda la app) siempre incluyó `parante_izq`/`parante_der`, así que TODAS las subdivisiones con Frente + Cenital tenían el mismo solape (Auto Coupé/Sedán/Familiar/Hatchback, Camioneta — todas las cabinas —, Utilitario Mediano/Grande, SUV Compacto/Grande). Augusto lo notó probando SUV Grande y pidió sacarlo de una vez en toda la app — cita: *"que quede solo en vista cenital, no en frente"*. Se borraron `parante_izq`/`parante_der` del `zonas.json` de Frente en las 13 subdivisiones afectadas; Cenital queda como única fuente de esas dos zonas en todos los casos. No hizo falta tocar ningún componente ni `index.js` — `ImageZoneDiagram`/`crearVistaDesdeZonas` arman `panelIds`/`panelLabels` directo desde cada `zonas.json`, así que sacar una zona ahí alcanza.

⚠️ **Bug distinto encontrado y resuelto en SUV (10 septiembre 2026)**: al revisar lo anterior, Augusto encontró que además `suv/compacto/cenital/` y `suv/grande/cenital/` tenían los parantes mal orientados — como bandas VERTICALES a la izquierda y derecha del techo (marcando en la práctica el arranque y el final del techo, no los costados), en vez de bandas HORIZONTALES arriba y abajo seguiendo el riel del techo real (que es como está bien hecho en Auto/Camioneta, confirmado revisando esos `zonas.json`). Se rehicieron las 2 zonas + se ajustó `techo` para no superponerse, calcado del criterio de Auto: banda angosta arriba, techo/vidrio en el medio, banda angosta abajo. En estas dos subdivisiones específicamente, Augusto pidió arriba = `parante_der`, abajo = `parante_izq` (al revés de cómo quedó en Auto/Camioneta, donde arriba = `parante_izq` y abajo = `parante_der` — la asignación depende de la orientación de cada referencia, no hay una regla universal, así que si se agregan más Cenital hay que chequear contra la imagen real de cada una, no copiar el criterio a ciegas).
...
- `auto/coupe/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital): capó, vidrio, óptica izq/der, espejo izq/der, frente/paragolpes). Vectorizado de un Audi A5 real usado solo como referencia de proporciones, sin logos.
...
- `auto/sedan/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), prácticamente calcadas de `auto/coupe/frente/` — misma proporción de referencia).
...
- `auto/familiar/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), misma imagen de referencia que Coupé/Sedán — el frente no cambia entre carrocerías en este set de fotos).
...
- `auto/hatchback/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital)). ⚠️ Referencia NUEVA (un VW Golf, no el Audi que veníamos usando para Coupé/Sedán/Familiar) — zonas hechas a medida, no calcadas.
...
- `camioneta/cabina_simple_chico/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que Auto: vidrio, capó, óptica izq/der, espejo izq/der, frente/paragolpes). Referencia: pickup chica tipo VW Gol/Voyage ("Saveiro").
...
- `camioneta/cabina_simple_mediano/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que Chico). Referencia: pickup mediana tipo Toyota Hilux cabina simple.
...
- `camioneta/doble_cabina_chico/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que el resto de Camioneta). Referencia: pickup chica doble cabina tipo Ford Maverick.
...
- `camioneta/doble_cabina_mediano/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que el resto de Camioneta). Referencia: pickup mediana doble cabina tipo Toyota Hilux.
...
- `camioneta/doble_cabina_grande/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que el resto de Camioneta). Referencia: pickup grande doble cabina tipo Ford F-150 Raptor.
...
- ⚠️ **`camioneta/utilitario_acarrozado_mediano` es la primera Camioneta con carrocería CERRADA** (furgón, referencia tipo Fiat Fiorino/similar) — a diferencia de Cabina simple y Doble cabina, que tienen caja de carga ABIERTA. Esto cambia el criterio de zonas en 3 de las 4 vistas:
  - `frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que el resto de Camioneta — el frente no cambia con el tipo de caja).
...
- `camioneta/utilitario_acarrozado_grande/` — nueva subdivisión (agregada 23 agosto 2026, referencia tipo furgón grande Mercedes Sprinter/similar). Mismo criterio de carrocería cerrada que Mediano, mismas 4 vistas:
  - `frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que el resto de Camioneta).
...
- `suv/compacto/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que el resto de la app). Referencia: SUV coupé compacto (techo con caída trasera, 4 puertas).
...
- `suv/grande/frente/` — listo (7 zonas (sin parantes, ver nota arriba — se marcan solo en Cenital), mismo criterio que el resto de la app). Referencia: SUV grande 3 filas tipo Toyota Fortuner/similar.
