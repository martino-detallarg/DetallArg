...
App móvil hecha con **Expo** (React Native), con **Supabase** (Postgres + Auth + Storage) como backend — conectado para autenticación, taller/horarios, clientes/vehículos, equipo e insumos/costos fijos (ver sección 6); `turnos`, `servicios` y el carrito de pedido todavía son mock en memoria. Todo el código es JavaScript plano (`.js` con JSX) — **no hay TypeScript** configurado en el proyecto.
...
│   ├── DataContext.js         # Context "resto", migrado a Supabase: misInsumos[] (con capacidadTotal/capacidadUnidad por insumo) y costosFijos[] + funciones CRUD async, más getInsumoById y descontarInsumos (consumo de stock por receta de servicio, ver sección 5)
...
│   ├── mockInsumos.js / mockTaller.js / mockServicios.js  # Datos iniciales de insumos, taller/planes/horarios y catálogo de servicios (mockTaller.js ya no incluye tallerInicial/misDatosIniciales; mockEquipo.js se borró al migrar EquipoContext, mockFinanzas.js se borró al sacarle el CHECK de categoría a costos_fijos, ver sección 6)
...
│   ├── FinanzasScreen.js                   # Costos del mes (dona: fijos + gastos variables reales) e Ingresos vs. Egresos de los últimos 6 meses (barras: cobros reales vs. costos fijos + gastos variables), en un pager de 2 páginas
...
- Un único `StatCard` centrado con la cantidad de turnos de hoy — el de "Ingresos del mes" (valor hardcodeado, no calculado) se sacó del todo, no solo se dejó de mostrar (a diferencia de Finanzas, que sí terminó calculando sus propios ingresos reales — ver Finanzas Fase A en sección 3, más abajo).
...
- Finanzas es un pager de 2 páginas (swipe horizontal, con puntos indicadores): "Costos del mes" (dona con Fijos vs. Variables, tocable para navegar a Costos Fijos) e "Ingresos vs. Egresos · últimos 6 meses" (barras). **Ya no son datos de ejemplo:** Finanzas Fase A (`data/FinanzasContext.js`) migró cobros y gastos variables a Supabase (tablas `cobros`/`gastos_variables`, mismo patrón sin actualización optimista que `DataContext`) — **corridas y verificadas contra la base real recién el 2026-09-07**: quedaron documentadas en `schema.sql`/`rls_tablas_negocio.sql` desde el commit `ab974fe` pero sin ejecutar, así que Finanzas Fase A completa estuvo rota en producción hasta esa fecha (ver `supabase/crear_cobros_y_gastos_variables.sql`). Los ingresos salen de `cobros`: se registran desde `TrabajoDetalleModal` → botón "Registrar cobro" → `RegistrarCobroModal`, que **admite pagos parciales** (un turno puede tener varios cobros asociados; `calcularSaldoPendienteTurno` en `utils/calculosFinanzas.js` suma todos contra `turno.precio`) — los turnos Finalizado/Entregado con saldo pendiente aparecen en `CuentasPorCobrarScreen.js` (Fiado, accedida desde Finanzas) y como `TrabajoPendienteCobroCard` en Notificaciones. Los "costos variables" salen de `gastosVariables` (se cargan desde `FinanzasScreen` → botón "Cargar gasto" → `GastoVariableModal`, con categoría y borrado; `'insumo_perdido'` es una categoría 100% automática — la app la inserta sola cuando el taller ajusta a mano el nivel de un insumo hacia abajo y ese insumo tiene `precio_compra` cargado, nunca aparece como chip elegible a mano); el total de costos fijos sigue saliendo de `DataContext`. Ambas tablas tienen `facturado` (boolean, con/sin comprobante fiscal formal, distinto de "cobrado") para el PDF "para el contador"; `gastos_variables` además tiene `comprobante_storage_path` (bucket privado `comprobantes-gastos`) para adjuntar el ticket/factura. La dona y las barras muestran cada una un overlay (spinner o mensaje de error) mientras cualquiera de sus fuentes (`cargandoCobros`/`cargandoGastosVariables`/`cargandoCostosFijos`, o sus respectivos `errorCarga...`) todavía no resolvió, para no mostrar nunca un monto en $0 falso.
- Costos Fijos: alta/edición (`CostoFijoModal`)/borrado de costos fijos mensuales con nombre libre (`nombre`, texto libre, sin categorías fijas — se sacó el CHECK de `categoria`), con el total mensual destacado arriba.
...
- **Migración de los Contexts de negocio a Supabase:** `ClienteContext`, `EquipoContext` y `DataContext` (insumos/costos fijos) ya están migrados (ver sección 5 y 6). `TurnoContext` y `ServicioContext` siguen 100% en memoria (`useState` con datos mock) — persistencia de esos datos de negocio: se pierde al cerrar sesión o recargar la app, igual que antes. `PedidoContext` (carrito de pedido a proveedor) sigue siendo puramente estado de UI a propósito, sin tabla propia (ver sección 6).
...
- El estado de negocio no vive en un único Context genérico. Cada dominio tiene su propio Context + Provider, todos con la misma forma interna (`useState` en memoria, sin `useReducer` ni librería externa, `value` memoizado con `useMemo`):
  ...
  - **`data/DataContext.js`** (`useData()`): quedó como el Context "de lo que sobra" — hoy solo `misInsumos` y `costosFijos` (insumos del taller y costos fijos de Finanzas), con `agregarInsumo`, `getInsumoById`, `descontarInsumos`, `agregarCostoFijo`, `actualizarCostoFijo`, `eliminarCostoFijo`. No tiene clientes, autos ni turnos. Cada insumo suma `capacidadTotal`/`capacidadUnidad` (envase) además de `nivel` (0-100). **Migrado a Supabase** (tablas `insumos`/`costos_fijos`), mismo criterio sin actualización optimista que el resto: fetch inicial con `cargandoInsumos`/`cargandoCostosFijos` (ordenados por `nombre`, ninguna de las dos tablas tiene `created_at`), mutaciones `async` que tiran si Supabase falla. `descontarInsumos` es la única mutación que no es un CRUD de un solo registro — descuenta varios insumos a la vez con un `Promise.all` de `UPDATE`s en paralelo, y solo toca el estado local si **todos** confirman (si alguno falla, tira y ninguno se aplica en memoria, aunque en la base algún `UPDATE` previo del mismo lote ya haya quedado guardado — no hay una transacción real envolviendo el lote). `TurnoContext.actualizarEstadoTrabajo` ahora es `async` y espera (`await`) esta función antes de mover el turno a "Finalizado" — si falla, el turno no cambia de estado (`TrabajoDetalleModal.js` deshabilita los chips de estado mientras guarda y muestra un error si falla). El catálogo de referencia (`catalogoInsumos`, 478 productos) sigue estático en el JS del cliente, sin tabla propia — fuera de alcance a propósito, ver `supabase/schema.sql`.
...
**Backend / base de datos:** Supabase (Postgres) — ver sección 6 para el detalle completo del esquema. Hoy están conectados con CRUD real `talleres` + `horarios_atencion`, `clientes` + `vehiculos`, `empleados` e `insumos` + `costos_fijos`; el resto de las tablas (`servicios`, `servicio_receta_items`, `turnos`, `turno_danios`, `turno_fotos_danio`, `turno_receta_aplicada`) existen en la base pero ningún Context de negocio las lee ni escribe todavía.
...
**Esquema de base (`supabase/schema.sql`):** 13 tablas, ya corridas y verificadas contra el proyecto real (con RLS activado, sin políticas todavía salvo `talleres`). El CHECK de `turno_danios.tipos` incluye los 4 tipos de daño sumados después para Moto (`rasgada`/`grasa`/`quemado`/`trizado`, ver `supabase/alter_turno_danios_tipos_moto.sql`) — confirmado corrido contra la base real el 2026-09-08 (consulta directa a `pg_constraint`, no solo por estar en este archivo). Modela los 7 Contexts en memoria: `talleres` (Taller + "Mis Datos" del titular, fusionados; `id` = `auth.users.id`), `horarios_atencion`, `clientes`, `vehiculos`, `empleados`, `insumos`, `costos_fijos`, `servicios`, `servicio_receta_items` (receta viva y editable de un servicio), `turnos`, `turno_danios`, `turno_fotos_danio`, y `turno_receta_aplicada` (**copia congelada** de la receta al momento de "Finalizado" — ver el bloque de "Consumo de stock" en la sección 5, mismo principio aplicado a nivel SQL: `servicio_id`/`insumo_id` son punteros vivos, `turno_receta_aplicada` son filas físicas insertadas una sola vez, con nombre/unidad denormalizados). Quedan **fuera de este esquema a propósito**: el catálogo de insumos (`catalogoInsumos`, sigue estático en el JS del cliente) y el carrito de "pedido a proveedor" (`PedidoContext`, sigue siendo solo estado de UI).

**Pendiente — paso manual en el SQL Editor de Supabase:** al migrar `DataContext` (insumos/costos fijos), el CHECK de `insumos.categoria` seguía con las 6 categorías placeholder originales (`desengrasantes`, `shampoo`, etc.), incompatibles con las 10 categorías reales del catálogo de Augusto (`lavado_exterior`, `interior`, `pulido_correccion`, `proteccion_sellado`, `accesorios_consumibles`, `ceras`, `vidrios`, `llantas_neumaticos`, `apc_desengrasante`, `ceramicos`) — y la columna `ph` quedó huérfana (ya no la escribe `AgregarInsumoModal.js`). `schema.sql` ya refleja el estado final, pero el `ALTER TABLE` real (`supabase/alter_insumos_categorias_reales.sql`) todavía hay que correrlo a mano contra el proyecto de Supabase — sin eso, cualquier `agregarInsumo` real sigue fallando contra el CHECK viejo.
...
- **Onboarding de talleres nuevos** (`talleres.onboarding_completado`, `supabase/alter_talleres_onboarding.sql`, corrida y verificada contra la base real el 2026-09-08 vía PostgREST): default `true` en la columna (cuentas ya existentes quedan con el wizard saltado), `handle_new_user()` la inserta en `false` explícitamente para altas nuevas. Del lado de la app, `TallerContext.js` expone `onboardingCompletado`/`marcarOnboardingCompletado()` y `App.js` muestra `OnboardingWizard.js` (4 pasos) cuando es `false`. Antes del 2026-09-08 la columna no existía; `TallerContext.js` tenía (y sigue teniendo, como red de seguridad) un fallback `data.onboarding_completado ?? true` para ese período.
...
El resto de la "data" de negocio que sigue siendo mock (turnos y servicios), definida en archivos estáticos y repartida por dominio (ver sección 5):
...
- `data/mockInsumos.js`: **ya no es mock de datos del taller** — `misInsumosIniciales` se borró al migrar `DataContext` a Supabase (ver sección 5 y 6). Lo que queda es el catálogo de referencia real (`catalogoInsumos`, 478 productos investigados marca por marca: Vonixx, Vintex, Sonax, Meguiar's, Soft99, Koch-Chemie, Chemical Guys, Menzerna, Gyeon) más las constantes de categorías/estantería (`CATEGORIAS`, `PAGINAS_ESTANTERIA`, `ORDEN_CATEGORIAS`, `UMBRAL_STOCK_BAJO`, `UNIDADES_CAPACIDAD`) — el catálogo en sí sigue estático en el JS del cliente a propósito, sin tabla propia en Supabase.
...
- **Warning de React Navigation "Found screens with the same name nested inside one another" (MiTaller):** `navigation/DashboardNavigator.js` registra `<Drawer.Screen name="MiTaller" component={MiTallerStackNavigator}>`, y adentro de ese `MiTallerStackNavigator` la primera pantalla del Stack anidado también se llama `<MiTallerStack.Screen name="MiTaller" component={MiTallerScreen}>` — mismo nombre en dos niveles de anidamiento. Preexistente (commit `2867f87c`, "Agregar gesto nativo de swipe-back en las pantallas del Drawer", 25/08/2026), no relacionado con ningún port reciente. No rompe nada en la práctica, pero conviene resolverlo en una pasada aislada: renombrar la screen interna (ej. `MiTallerInicio`) y actualizar cualquier `navigation.navigate("MiTaller")` que apunte al nivel del Stack en vez de al del Drawer.
