...
- Finanzas es un pager de 2 páginas (swipe horizontal, con puntos indicadores): "Costos del mes" (dona con Fijos vs. Variables, tocable para navegar a Costos Fijos) e "Ingresos vs. Egresos · últimos 6 meses" (barras). **Ya no son datos de ejemplo:** Finanzas Fase A (`data/FinanzasContext.js`) migró cobros y gastos variables a Supabase (tablas `cobros`/`gastos_variables`, mismo patrón sin actualización optimista que `DataContext`) — los ingresos salen de `cobros` (se registran desde `TrabajoDetalleModal` → botón "Registrar cobro" → `RegistrarCobroModal`, un cobro por turno en v1, sin pagos parciales) y los "costos variables" de `gastosVariables` (se cargan desde `FinanzasScreen` → botón "Cargar gasto" → `GastoVariableModal`, con categoría y borrado); el total de costos fijos sigue saliendo de `DataContext`. La dona y las barras muestran cada una un overlay (spinner o mensaje de error) mientras cualquiera de sus fuentes (`cargandoCobros`/`cargandoGastosVariables`/`cargandoCostosFijos`, o sus respectivos `errorCarga...`) todavía no resolvió, para no mostrar nunca un monto en $0 falso.
