...
### Gráficos e íconos
- `react-native-svg` `15.12.1` — usado a mano (sin librería de charts) en varios lugares: `components/wizard/FuelGauge.js` (medidor de nafta, arco SVG con gesto táctil vía `PanResponder`), `components/GraficoDonut.js` y `components/GraficoBarras.js` (gráficos de la pantalla Finanzas), `components/CircularProgress.js` (anillo de progreso genérico de Home, ver sección 3), y los diagramas de check-in visual — el genérico de Frente (`components/wizard/DamageDiagram.js`), el histórico de paneles vectorizados a mano (`components/diagrams/vehicles/PickupCabinaSimpleDiagram.js`, legacy) y el registro nuevo de diagramas "imagen + zonas" por carrocería real (`components/diagrams/vehicles/ImageZoneDiagram.js` + `index.js`, ver sección 2 y 4).
...
- Sin Supabase, Firebase, ni ningún SDK de backend **en `main`** — sí hay una migración a Supabase avanzada en la branch `feature-supabase`, todavía sin mergear (ver sección 11).
...
│   └── checkin-diagrams/        # Pipeline de assets del check-in visual: un chat de diseño gráfico aparte va entregando acá, por tipo/subdivisión/vista, diagrama.png (silueta sin logo) + zonas.json (polígonos tocables) + imagen.js (mismo PNG en base64, ver nota de bug más abajo). Tiene su propio README.md con el detalle completo: convención de carpetas, formato de zonas.json, criterio de qué panel va en qué vista, y una bitácora de bugs de proceso ya resueltos (recorte al cargar con require() de PNG — de ahí el base64 embebido en vez de require(); pérdida de ruedas al rellenar contornos) — Augusto es quien integra cada entrega en components/diagrams/vehicles/ (ver ahí y sección 4)
│
...
│
├── components/diagrams/vehicles/   # Registro de diagramas de paneles reales por tipo/subdivisión de carrocería (armado por Augusto sobre los assets que va entregando el pipeline de diseño de assets/checkin-diagrams/, ver sección 4)
│   ├── index.js                      # DIAGRAMAS_POR_TIPO_VEHICULO (mapeo tipo de vehículo -> { vistas }) + obtenerClaveDiagrama({ tipoVehiculo, grupo, subdivision }); si la combinación no está acá, InspeccionVisualStep cae al genérico de una sola vista (DamageDiagram.js)
│   ├── ImageZoneDiagram.js            # Componente genérico "imagen + zonas": pinta una imagen de referencia (silueta sin logo, base64 embebido) con un <Polygon> tocable por zona encima (react-native-svg). `crearVistaDesdeZonas(vistaId, imageSource, zonasJson)` arma cada vista concreta a partir de un diagrama.png+zonas.json procesado; namespacea cada id de zona como `${vistaId}__${id}` porque nombres de zona genéricos (ej. "vidrio") se reusan entre vistas para paneles físicamente distintos. **Tiene un `debugBorde` (borde rojo 2px) y un `console.log` en el `onLayout` marcados como "DEBUG temporal" en el propio código, todavía sin sacar — ver sección 9.**
│   ├── autoCoupe.js / autoSedan.js / autoDescapotable.js / autoFamiliar.js / autoHatchback.js   # Las 5 subdivisiones de Auto, familia completa — cada una arma sus vistas (Frente/Atrás/Lateral/Cenital, salvo Descapotable que no tiene Cenital) con `crearVistaDesdeZonas`
│   ├── camionetaCabinaSimpleChico.js / camionetaCabinaSimpleMediano.js   # Camioneta/Cabina simple, las 2 subdivisiones completas (4 vistas c/u)
│   ├── camionetaDobleCabinaChico.js / camionetaDobleCabinaMediano.js / camionetaDobleCabinaGrande.js   # Camioneta/Doble cabina, las 3 subdivisiones completas (4 vistas c/u) — familia Doble cabina terminada
│   ├── camionetaUtilitarioMediano.js / camionetaUtilitarioGrande.js   # Camioneta/Utilitario acarrozado, Mediano y Grande completos (4 vistas c/u); falta "Chico" (subdivisión nueva sin referencia todavía, cae al genérico)
│   ├── camionetaUtilitarioChico.js    # **Archivo huérfano:** era el diagrama original de "Utilitario acarrozado/Chico", pero ese contenido se renombró a Mediano (ver nota del 23/08/2026 en el README de assets/checkin-diagrams) — `index.js` ya no lo importa ni lo registra. Queda pendiente de borrado manual, ver sección 9.
│   ├── suvCompacto.js / suvGrande.js   # SUV, las 2 subdivisiones completas (4 vistas c/u) — familia SUV terminada
│   ├── PickupCabinaSimpleDiagram.js   # Diagrama legacy (12 paneles vectorizados a mano, sin distinguir Chico/Mediano) — ya reemplazado por camionetaCabinaSimpleChico.js/Mediano.js; sigue registrado solo como resguardo transitorio en obtenerClaveDiagrama para cuando `grupo === "Cabina simple"` pero `subdivision` todavía no se eligió
│   └── img/                            # Imágenes base64 de auto/Coupé (autoCoupe*Imagen.js) — las demás carrocerías importan su imagen.js directo desde assets/checkin-diagrams/, sin copia local acá
│
├── screens/
...
- Dos `StatCard`: cantidad de turnos de hoy e ingresos del mes (este último es un número fijo hardcodeado, no calculado — ver sección 4). El número protagonista de cada `StatCard` usa `fonts.heading` (Archivo) en vez de JetBrains Mono (que quedaba "de código"); Mono se mantiene solo para la etiqueta chica de arriba. La card "Turnos de hoy" suma un `components/CircularProgress.js` (anillo SVG reutilizable, mismo enfoque que `GraficoDonut`) con turnos Finalizados/Entregados sobre el total de hoy — es solo un cálculo visual para el anillo, no cambia la lógica de turnos; "Ingresos del mes" se queda sin anillo por no tener una meta real definida.
...
- Paso "Inspección Visual" (`InspeccionVisualStep.js`): diagrama de daños de check-in visual, en un carrusel de vistas. Las vistas del carrusel ya no son una lista fija: se derivan dinámicamente del registro `DIAGRAMAS_POR_TIPO_VEHICULO` (`components/diagrams/vehicles/index.js`) según la combinación tipo/grupo/subdivisión elegida en el paso anterior — si esa combinación tiene diagrama propio, el carrusel muestra sus vistas reales (Frente/Atrás/Lateral/Cenital, según la carrocería; Auto/Descapotable solo tiene 3, sin Cenital); si no, cae a una única vista genérica de 7 zonas (`DamageDiagram.js`, solo "Frente" con una foto de referencia de fondo). Hoy **14 de las 18 subdivisiones** del árbol de carrocerías tienen diagrama real de paneles vectorizados sobre imagen (`components/diagrams/vehicles/ImageZoneDiagram.js`, ver sección 2 y 4): toda la familia Auto (Coupé, Sedán, Descapotable, Familiar, Hatchback), toda Camioneta salvo "Utilitario acarrozado/Chico" (Cabina simple Chico/Mediano, Doble cabina Chico/Mediano/Grande, Utilitario acarrozado Mediano/Grande), y toda SUV (Compacto, Grande) — quedan pendientes ese "Chico" de Utilitario y las 3 subdivisiones de Moto (naked/sport/motocross, ver sección 4). Cada zona/panel admite **varios tipos de daño a la vez** (rayón, abolladura, óxido, **rasgada** —tipo nuevo pensado para lonas de caja, ver `data/tiposDanio.js`—, etc., más "otro" con nota de texto libre), y hay una lista resumen "Daños registrados" debajo. También se puede adjuntar foto del daño desde la galería, habilitado solo si ya se marcó al menos un daño.
...
- Los modales de formulario que no lo tenían (Cliente, Vehículo, Empleado, Costo Fijo, Servicio, Editar Taller, Agregar Insumo) y el paso de Inspección Visual suman `KeyboardAvoidingView`, para que el teclado no tape el campo que se está completando.
- Menú lateral (`DrawerContent.js`): íconos y textos de los ítems pasaron a `colors.textPrimary` con tamaño mayor (antes tenían menos contraste contra el fondo oscuro).
...
- **Diagrama de daños específico por carrocería, 14 de 18 subdivisiones:** ya no es mayormente genérico — toda la familia Auto (5 subdivisiones), toda Camioneta salvo "Utilitario acarrozado/Chico" (7 de 8 subdivisiones) y toda SUV (2 subdivisiones) tienen diagrama real de paneles vectorizados sobre imagen, con 3 o 4 vistas cada una (ver detalle en sección 2 y 3). Quedan pendientes: "Utilitario acarrozado/Chico" (sigue seleccionable en `TipoVehiculoStep.js`, pero cae al genérico porque todavía no tiene referencia procesada — no confundir con `camionetaUtilitarioChico.js`, que es un archivo viejo huérfano de una subdivisión que se renombró a Mediano, ver sección 9) y las 3 subdivisiones de Moto (Naked/Sport/Motocross — ya seleccionables en `TipoVehiculoStep.js`, pero sin ningún diagrama propio todavía, caen al genérico). El pipeline de assets que alimenta esto (`assets/checkin-diagrams/`, ver sección 2) lo mantiene un chat de diseño gráfico aparte, con su propio README de bitácora.
...
- **Debug sin sacar en `ImageZoneDiagram.js`:** el componente que renderiza todos los diagramas de paneles reales (14 subdivisiones, ver sección 4) todavía tiene un `debugBorde` (borde rojo de 2px alrededor de cada diagrama) y un `console.log` en su `onLayout`, ambos marcados en el propio código como "DEBUG temporal — sacar... una vez confirmado que el contenedor mide lo esperado". Visualmente notorio (el borde rojo se ve en cualquier diagrama de check-in) — pendiente de limpieza.
- **Archivos huérfanos de la migración "Chico" → "Mediano" de Utilitario acarrozado:** `components/diagrams/vehicles/camionetaUtilitarioChico.js` y la carpeta `assets/checkin-diagrams/camioneta/utilitario_acarrozado_chico/` quedaron sin ningún uso — el contenido real se renombró a `utilitario_acarrozado_mediano/`/`camionetaUtilitarioMediano.js` el 23/08/2026 (ver sección 2 y 4), y `index.js` ya no los importa. El propio README de `assets/checkin-diagrams/` deja la nota de que hay que borrarlos a mano porque el chat de diseño que lo detectó no tiene forma de borrar archivos del repo de Augusto.
...
## 11. Trabajo en curso: migración a Supabase (branch `feature-supabase`, sin mergear a `main`)

**Todo lo de arriba describe el estado real de `main`, que sigue 100% mock/en memoria (sección 6).** En paralelo, Nico viene reestructurando los Contexts para que hablen contra un backend real (Supabase/Postgres) en la branch `origin/feature-supabase`, que diverge de `main` en el commit `6f02538` (antes de los diagramas nuevos de Augusto y del pulido visual de Home/drawer de las secciones 2-4 y 9 de arriba, que todavía no existen en esa branch). Resumen de lo ya hecho ahí, de más viejo a más nuevo:

- **Cliente y esquema:** `lib/supabase.js` (cliente `@supabase/supabase-js`, con `AsyncStorage` para persistir sesión y `react-native-url-polyfill` porque Hermes no trae `URL` completo), leyendo `EXPO_PUBLIC_SUPABASE_URL`/`EXPO_PUBLIC_SUPABASE_ANON_KEY` de un `.env` no versionado. `supabase/schema.sql`: 13 tablas modelando los 7 Context de negocio de siempre (`talleres` fusiona Taller + Mis Datos, más `horarios_atencion`, `clientes`+`vehiculos`, `empleados`, `insumos`, `costos_fijos`, `servicios`+`servicio_receta_items`, `turnos`+`turno_danios`+`turno_fotos_danio`+`turno_receta_aplicada` como copia congelada). El catálogo de insumos y el carrito de pedido a proveedor quedan fuera a propósito, siguen siendo solo UI/estático.
- **Autenticación real:** `data/AuthContext.js` (`useAuth()`) reemplaza el login/signup simulado — envuelve **toda** `App.js` (no solo la parte logueada), con `signIn`/`signUp`/`signOut`/`resendConfirmation`/`solicitarRecuperacion`/`confirmarRecuperacion` sobre `supabase.auth.*`, persistencia de sesión entre reinicios, y un trigger de alta (`handle_new_user`) que crea la fila de `talleres` al registrarse. Recuperación de contraseña con código OTP de 6 dígitos (`OlvidePasswordScreen.js` → `RestablecerPasswordScreen.js`) en vez de link mágico, porque Expo Go no maneja deep links de forma confiable sin build de EAS.
- **RLS:** 42 políticas en total sobre las 13 tablas (`supabase/trigger_nuevo_usuario.sql` + `supabase/rls_tablas_negocio.sql`), con mínimo privilegio — solo se habilita una operación si ya hay una función real en el Context que la dispare (ej. sin `DELETE` en `insumos`/`turnos`, `turno_receta_aplicada` solo `SELECT`/`INSERT` para reforzar a nivel de base que el snapshot congelado nunca se toca).
- **Contexts de negocio migrados del todo (lectura + escritura real, sin actualización optimista):** `ClienteContext` (clientes + vehículos, con embedded resource de PostgREST) y `EquipoContext` (empleados). `TallerContext` migrado parcial: `talleres` (nombre/logo/misDatos/plan) y `horarios_atencion` sí leen y escriben de verdad — incluido el logo, que ahora se sube a Supabase Storage (bucket público `logos`, un archivo por taller con cache-busting) —, pero `cambiarPlan` sigue siendo puramente client-side (sin pagos reales). `TurnoContext`, `ServicioContext`, `DataContext` (insumos/costos fijos) y `PedidoContext` **siguen 100% en memoria**, sin tocar.
- **Estados de carga y error:** componente nuevo `EstadoCarga.js` (spinner / error + reintentar / contenido) y `utils/errores.js`, reemplazando los `console.warn` silenciosos que había antes en los 4 fetches iniciales migrados (Taller, Horarios, Clientes, Equipo). De paso se corrigió un bug real: `MisDatosScreen.js` se quedaba pegado a datos vacíos si se entraba antes de que terminara de cargar `TallerContext`.
- **Variables de entorno nuevas en esa branch:** `EXPO_PUBLIC_SUPABASE_URL` y `EXPO_PUBLIC_SUPABASE_ANON_KEY` en `.env` (no versionado) son obligatorias para levantar el proyecto ahí — sin esto la app tira un error explícito al arrancar. En `main` esto no aplica todavía (sección 7).

**Qué falta para poder mergear a `main` sin perder nada:** reconciliar esta branch con los diagramas de vehículos y el pulido visual que se sumaron a `main` después del punto de divergencia (secciones 2-4 y 9), y terminar de migrar `TurnoContext`/`ServicioContext`/`DataContext`/`PedidoContext` si se quiere que el logout deje de resetear esos datos (hoy en `feature-supabase` el logout ya no borra `talleres`/`clientes`/`vehículos`/`empleados`, pero sí sigue reseteando turnos/servicios/insumos/pedido, igual que en `main`).
