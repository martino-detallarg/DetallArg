...
- `data/mockFinanzas.js`: **ya no exporta datos del taller** — `costosFijosIniciales` se borró al migrar `DataContext` a Supabase. Lo que queda son las constantes de categorías (`CATEGORIAS_COSTOS_FIJOS`, `ORDEN_CATEGORIAS_COSTOS_FIJOS`) para Costos Fijos.
...
- **`ServicioModal.js` sin actualizar tras la propuesta de esquema de servicios (hueco conocido y tolerado):** cuando se corra el DDL de `docs/propuesta-servicios-insumos/DDL.sql` y se apliquen `ServicioContext.patch`/`DataContext.patch` (rama `propuesta-servicios-insumos`, ver `PROPUESTA.md` de esa carpeta), `agregarServicio`/`editarServicio` van a esperar `descripcion`/`duracionValor`/`duracionUnidad` en vez de `categoria`/`duracionEstimada`. `ServicioModal.js` no se toca en esa tanda — sigue mandando los campos viejos, que la función nueva simplemente ignora. No rompe (los campos nuevos son `nullable` en la base), pero **"Agregar/editar servicio" va a guardar `descripcion` y duración en `null`** hasta que el modal se actualice. A diferencia del gap equivalente que tenía `AgregarInsumoModal.js` (ver `DataContext-compat-shim.patch`, ese sí rompía el alta y se resolvió con un shim temporal), acá se decidió **no** sumar un shim: Augusto ya tiene identificado este rediseño de `ServicioModal.js` en su propia propuesta ("tarjetas, sin categoría, descripción y duración en horas/días") y va a traer el arreglo real — sumar un parche temporal acá sería trabajo duplicado.
