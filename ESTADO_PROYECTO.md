...
**`supabase/alter_insumos_categorias_reales.sql` (corrido y verificado contra la base real el 2026-09-08 vía PostgREST — `insumos.ph` ya no existe):** al migrar `DataContext` (insumos/costos fijos), el CHECK de `insumos.categoria` seguía con las 6 categorías placeholder originales (`desengrasantes`, `shampoo`, etc.), incompatibles con las 10 categorías reales del catálogo de Augusto (`lavado_exterior`, `interior`, `pulido_correccion`, `proteccion_sellado`, `accesorios_consumibles`, `ceras`, `vidrios`, `llantas_neumaticos`, `apc_desengrasante`, `ceramicos`) — y la columna `ph` quedó huérfana (ya no la escribe `AgregarInsumoModal.js`). `schema.sql` ya reflejaba el estado final desde antes, y ahora el `ALTER TABLE` real también está confirmado corrido contra el proyecto de Supabase.
