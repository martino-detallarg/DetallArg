...
- **Migración de los Contexts de negocio a Supabase:** `ClienteContext`, `TurnoContext`, `ServicioContext`, `EquipoContext`, `DataContext` (insumos/costos fijos) y `PedidoContext` siguen 100% en memoria (`useState` con datos mock) — solo la autenticación y el bootstrap de lectura de `TallerContext` están conectados a Supabase (ver sección 6). Persistencia de datos de negocio: se pierde al cerrar sesión o recargar la app, igual que antes.
...
- **Al cerrar sesión se destruye todo el estado en memoria de los 7 Providers de negocio** (se desmontan al salir de `pantalla === "app"`), y al volver a loguearse cada uno arranca de cero con sus datos iniciales — `ClienteContext` arranca con 2 clientes de ejemplo (`c1`, `c2`) y sus vehículos anidados (`a1`, `a2`, `a3`), y `TurnoContext` arranca con `turnosIniciales` de `mockData.js` (ver sección 6). Los IDs de ambos sets de datos de ejemplo están coordinados a propósito para que los turnos de ejemplo muestren cliente y vehículo reales. `EquipoContext` arranca vacío (coherente con el plan Básico por defecto, que no permite ningún empleado). **`TallerContext` es la única excepción parcial**: como su estado inicial ahora viene de un `SELECT` real a Supabase (no de un mock), sus datos de base sobreviven al logout — pero cualquier edición hecha en la sesión (`actualizarTaller`, `actualizarMisDatos`, etc.) sigue sin persistirse a ningún lado, así que esas ediciones puntuales sí se pierden igual que en el resto de los Contexts.
...
**Supabase (Postgres + Auth) está conectado**, pero solo para dos cosas: autenticación real y la lectura inicial de la tabla `talleres`. El resto de los datos de negocio (clientes, turnos, servicios, insumos, equipo, pedido) siguen siendo 100% mock en memoria — ver sección 5 y el detalle de cada Context.
...
- **RLS mínima, solo para `talleres`:** `authenticated` puede `SELECT`/`UPDATE` únicamente su propia fila (`auth.uid() = id`). Sin policy de `INSERT` (la única vía de alta es el trigger) ni de `DELETE`. Las otras 12 tablas del esquema **siguen sin ninguna policy** — RLS activado pero bloqueado para todos, incluso el dueño — hasta que se aborde ese trabajo aparte.
...
Toda la "data" de negocio sigue siendo mock, definida en archivos estáticos y repartida por dominio (ver sección 5):
...
**Pendiente para cuando se migren también los Contexts de negocio a Supabase:** hoy "Cerrar sesión" sigue reseteando **todos los datos de negocio** a los valores de fábrica de cada mock (los 7 Providers se desmontan al salir de `pantalla === "app"`) — es el comportamiento esperado mientras esos Contexts sigan en memoria. La autenticación en sí y los datos base de `talleres` ya NO se pierden (sobreviven en Supabase), pero las ediciones de `TallerContext` hechas en la sesión tampoco se guardan todavía (ver nota en sección 5). Cuando se migre cada Context, hay que asegurarse de que "cerrar sesión" siga significando solo "cerrar el acceso" y no borre ni resetee ningún dato de negocio real.
...
6. **Hace falta un archivo `.env`** con `EXPO_PUBLIC_SUPABASE_URL` y `EXPO_PUBLIC_SUPABASE_ANON_KEY` de un proyecto de Supabase real, con el esquema de `supabase/schema.sql` + `supabase/trigger_nuevo_usuario.sql` ya corridos (ver sección 7) — sin esto la app no arranca. El resto de los datos de negocio (clientes, turnos, etc.) sigue siendo mock, sin nada más que configurar.
