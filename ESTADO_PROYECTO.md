...
- **Al cerrar sesión se destruye todo el estado en memoria** (los cinco Providers se desmontan al salir de `pantalla === "app"`), y al volver a loguearse cada Context arranca de cero con sus datos iniciales — `ClienteContext` arranca con 2 clientes de ejemplo (`c1`, `c2`) y sus vehículos anidados (`a1`, `a2`, `a3`), y `TurnoContext` arranca con `turnosIniciales` de `mockData.js` (ver sección 6). Los IDs de ambos sets de datos de ejemplo están coordinados a propósito para que los turnos de ejemplo muestren cliente y vehículo reales.
